...
export default function LikedPosts() {
  ...
  useEffect(() => {
    const getLikedPosts = async () => {
      try {
        const res = await fetch(`http://localhost:5000/api/users/${user.id}/liked-posts`);
        const data = await res.json();
        setPosts(data);
      } catch (err) { console.error(err); }
      finally { setIsLoading(false); }
    };
    if (user?.id) getLikedPosts();
  }, [user?.id]);
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-950 to-slate-900 text-slate-100 flex flex-col font-sans">
      ...
      <main className="flex-1 max-w-4xl mx-auto w-full px-4 md:px-6 py-8">
        <div className="flex items-center gap-3 mb-8">
          ...
          <h1 className="text-2xl font-bold text-white">좋아요한 글 ({posts.length})</h1>
        </div>
        {isLoading ? (
            ...
            {posts.map(post => (
              <div key={post.id} onClick={() => navigate(`/post/${post.id}`)}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-slate-500 mt-1">{post.category} · {post.author} · ❤️ {post.likes_count}</p>
                </div>
          ...
          <div className="text-center py-20 text-slate-600 border border-dashed border-slate-800/60 rounded-2xl">좋아요한 게시글이 없습니다.</div>
        )}
      </main>
